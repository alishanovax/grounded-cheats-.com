# Deploy groundedcheats.com

Step-by-step guide to deploy the Grounded Cheats static site to **groundedcheats.com** on Cloudflare Pages, configure DNS, and submit to Google Search Console.

## Prerequisites

- Node.js **≥ 22.12.0**
- Cloudflare account with access to **groundedcheats.com** DNS
...
node scripts/generate-grounded-blog-posts.mjs
...
3. Configure build settings:
   - **Project name:** `groundedscheats` (existing) or create a new project
...
This runs `wrangler pages deploy dist --project-name=groundedcheats` (see `wrangler.toml`).

## 3. Custom domain and DNS

Add **groundedcheats.com** as the primary custom domain on the Pages project.

### Apex (groundedcheats.com)
...
2. In **Rules** → **Redirect Rules** (or Bulk Redirects), create:
   - **Source:** `www.groundedcheats.com/*`
   - **Target:** `https://groundedcheats.com/${1}`
   - **Status:** 301

The deployed `functions/_middleware.js` also enforces apex canonical host, legacy domain redirects (`groundedcheats.com`, `.net`, `.com`), and legacy path redirects.
...
- `https://groundedcheats.com/es/`
- `https://groundedcheats.com/grounded-cheats/`
- `https://groundedcheats.com/grounded-trainer/`
- `https://groundedcheats.com/sitemap.xml`
- `https://groundedcheats.com/robots.txt`

Verify redirects:

- `http://groundedcheats.com` → `https://groundedcheats.com` (301)
- `https://www.groundedcheats.com` → `https://groundedcheats.com` (301)
- Legacy domains (e.g. `groundedcheats.com`) → `https://groundedcheats.com` (301)
- `/sitemap-index.xml` → `/sitemap.xml` (301)
- Legacy paths (e.g. `/fortnite-hacks/`) → Grounded equivalents (301)

## 5. Google Search Console

1. Go to [Google Search Console](https://search.google.com/search-console).
2. **Add property** → choose **Domain** → enter `groundedcheats.com`.
...
4. After verification, open **Sitemaps** and submit:
   ...
   https://groundedcheats.com/sitemap.xml
   ```
   Remove any legacy submissions (`sitemap-index.xml`, old `groundedcheats.com` URLs).
5. Use **URL Inspection** to request indexing for:
   - Homepage (`/`)
   - Pillar page (`/grounded-cheats/`)
   - Key landing pages (`/grounded-trainer/`, `/grounded-god-mode/`, `/grounded-cheats-2026/`, etc.)
...
| Regenerate blog posts | `node scripts/generate-grounded-blog-posts.mjs` |
...
- [ ] Custom domain `groundedcheats.com` attached and active
- [ ] `www` redirects to apex
- [ ] Legacy domains 301 to `groundedcheats.com`
- [ ] Always Use HTTPS enabled
- [ ] `robots.txt` and sitemaps serve from `https://groundedcheats.com`
- [ ] Google Search Console domain verified
- [ ] `sitemap.xml` submitted in GSC
- [ ] Homepage and `/grounded-cheats/` requested for indexing
