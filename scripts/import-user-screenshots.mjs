...
const assetsDir =
	'C:/Users/Aman/.cursor/projects/c-Users-Aman-groundedhacks/assets';
...
const USER_SCREENSHOTS = [
	{
		...
		file: 'grounded-god-mode-player-tags.webp',
	},
	{
		source:
		...
		file: 'grounded-cheat-engine-skeleton.webp',
	},
	{
		source:
		...
		file: 'grounded-trainer-sniper.webp',
	},
	{
		source:
		...
		file: 'grounded-trainer-skeleton.webp',
	},
	{
		source:
		...
		file: 'grounded-god-mode-radar.webp',
	},
	{
		source:
		...
		file: 'grounded-cheats-combat.webp',
	},
];
...
const REMOVE_PREFIXES = [
	'grounded-battle-royale-combat',
	'grounded-base camp-fight',
	'grounded-cheats-trainer',
	'grounded-cheats-cover',
	'grounded-cheats-esp-god mode',
	'grounded-cheats-hero',
	'grounded-cheats-logo',
	'grounded-cheats-package',
	'grounded-header-art',
	'grounded-loadout-builder',
	'grounded-player-esp',
	'grounded-scav-run-combat',
	'grounded-scav-run-mode',
	'grounded-squad-fight',
	'grounded-verdansk-map',
];

async function removeOldGroundedImages() {
...
async function generateResponsiveVariants() {
	const heroSource = path.join(imagesDir, 'grounded-god-mode-player-tags.webp');
	const heroMeta = await sharp(heroSource).metadata();

	for (const width of HERO_WIDTHS) {
		if (heroMeta.width && width > heroMeta.width) continue;
		const file = `grounded-god-mode-player-tags-${width}w.webp`;
	...
	for (const { file } of USER_SCREENSHOTS) {
		if (file === 'grounded-god-mode-player-tags.webp') continue;
...
async function generateFavicons() {
	const logoPath = path.join(imagesDir, 'grounded-cheats-logo.png');
